...
type Props = {
  ...
  columnIndex?: number; // 0-based column within a day for overlap layout
  totalColumnsInGroup?: number; // total sibling columns in this overlap group
  onClick: (item: TimelineItem, rect: DOMRect) => void;
};

export function TimelineBar({ item, dayStart, color, isColumnLayout = false, allDayIndex = 0, columnIndex = 0, totalColumnsInGroup = 1, onClick }: Props) {
  ...
  if (isColumnLayout) {
    ...
    // Compute horizontal position as percentages within the day column
    const pct = 100 / totalColumnsInGroup;
    const barLeft = `${columnIndex * pct}%`;
    const barWidth = `${pct}%`;
    const inset = totalColumnsInGroup > 1 ? 2 : 4;
    ...
    return (
      ...
      >
        <div
          className="h-full w-full rounded overflow-hidden flex flex-col justify-start px-2 py-2 transition-opacity group-hover:opacity-90"
          style={{
            backgroundColor: color + '26',
            borderLeft: `4px solid ${color}`,
          }}
