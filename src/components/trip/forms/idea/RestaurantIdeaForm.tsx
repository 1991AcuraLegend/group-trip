...
type Props = { tripId: string; onClose: () => void; existingIdea?: Restaurant };

export function RestaurantIdeaForm({ tripId, onClose, existingIdea }: Props) {
  ...
  async function onSubmit(data: FormValues) {
    ...
    try {
      ...
      setSubmitError(error instanceof Error ? error.message : 'Failed to save restaurant idea');
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-3">
      ...
      />
      <Input label="Cuisine type (optional)" placeholder="Italian, Japanese..." {...register('cuisine')} />
